...
| `data/cases.json` | 업무분야 페이지 "주요 수행사례" — 배열이 비어 있으면 화면에 나타나지 않는다. 실제 수행 사건만, 의뢰인 특정 정보 없이 수록. 형식은 파일 안 `_format` 참고 |
| `data/qna.json` | 질의응답 목록 — 실명·연락처·동호수 금지 |
| `data/registry.json` | 등기센터 세율·채권요율·단지 목록 |
