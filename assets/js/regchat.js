/* 등기센터 안내 도우미 — 제이엘 등기센터(동탄) 전용

   이 페이지에는 이미 접수·조회·계산기·서류 네 가지 도구가 있는데,
   처음 온 사람은 자기 상황이 어디에 해당하는지를 먼저 모른다.
   그래서 정해진 문답으로 물어보고 해당 도구까지 데려다주는 역할만 한다.

   상담은 받지 않는다. 문의 남기기 폼도 없다.
   상담은 전화(대표번호)와 카카오톡으로만 받는다 — 창구를 늘리지 않는다.
   서버가 없고, 아무것도 입력받지 않는다.
*/
(function () {
  'use strict';
  ...
  var NODES = {
    ...
    group: {
      label: '아파트 단체등기 문의예요',
      say: ['아파트 전체를 함께 진행하시는 단체등기는 전화로 받습니다.',
            '단지명과 입주 시기를 알려 주시면 담당자가 절차와 비용을 안내해 드립니다.'],
  ...
  root.innerHTML =
    '<button type="button" class="rchat__fab" aria-expanded="false">' +
      '<span class="rchat__fabi" aria-hidden="true"></span>등기 안내' +
    '</button>' +
    '<section class="rchat__box" hidden aria-label="등기 안내">' +
      '<header class="rchat__head">' +
        '<div><strong>제이엘 등기센터</strong><span>자주 묻는 안내</span></div>' +
